...
def decoder_funct(filename, content):
    ...
    output_directory = os.path.join(os.getcwd(), "Предзаказы")
...
for act_num in df_postavki['номер поставки']:
    ...
    url = f"https://seller-supply.wildberries.ru/ns/sm-supply/supply-manager/api/v1/supply/detailsXls"
    ...
    if response.status_code != 200:
        print(response, 'err_step_1', response.content)
    ...
    if 'error' in response_result:
        ...
        name = str(act_num)
        file = response_result['result']['file']
